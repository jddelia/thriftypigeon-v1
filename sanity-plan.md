...
1. **Project provisioning**
   - **Install tooling**: ensure Node.js ≥ 18 is installed locally, then install the Sanity CLI globally via `npm install -g @sanity/cli` (or run through `npx`).
   - **Authenticate**: run `sanity login` and follow the browser prompt to authorize your Sanity account.
   - **Create or select project**:
     - Run `npm create sanity@latest` from a clean directory.
     - Choose "Create new project" (or "Use existing" if you already provisioned one in the dashboard) and give it an intuitive name such as `thriftypigeon`.
     - Accept the default `production` dataset or create a dedicated dataset for staging if required.
     - When prompted, generate the project locally (Studio template optional) or skip if Studio will live inside this repo.
   - **Dashboard verification**: visit [https://sanity.io/manage](https://sanity.io/manage) to confirm the project & dataset exist and note the `projectId` and dataset name.
   - **API tokens**: create a *Read* token for runtime use and an optional *Write* token for migration/import scripts. Store tokens securely (1Password, env vars) and never commit them.
   - **CORS origins**: in the Sanity project settings, add your local dev URL (e.g., `http://localhost:3000`) and deployment domain (e.g., `https://thriftypigeon.com`) to the CORS allowed origins. Enable credentials for preview usage.
   - **Team access**: invite any collaborators who need Studio access and assign roles (Editor vs. Developer) as appropriate.
2. **Schema definition**
